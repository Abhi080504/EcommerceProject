...
const Account = () => {
    ...
    React.useEffect(() => {
        if (!user) {
            // If user is null, we try to load it. 
            // If loading fails (e.g. 401), we should probably redirect, 
            // but dispatch(loadUserProfile()) is async.
            // For now, let's just dispatch it. 
            // The App.tsx also dispatches it, so this might be redundant but safe.
        ...
        // Note: If auth fails, the user remains null. 
        // We might want to handle redirect to home if auth fails definitively.
        // But removing the !jwt check fixes the immediate "redirect on refresh" loop.
