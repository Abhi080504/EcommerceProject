...
const DrawerList = ({ menu, menu2, toggleDrawer }: DrawerListProp) => {
  ...
  const renderMenuItem = (item: menuItem, index: number, isLogout: boolean = false) => {
      ...
      >
        <div
          className={`flex items-center gap-4 py-3 pl-4 pr-6 rounded-r-[999px] transition-all duration-150
            ${isLogout 
              ? "text-[#ff6b6b] hover:bg-red-500/10 hover:text-red-400" 
              : active 
                ? "bg-[#0f9f8f] text-white" 
                : "text-[#66ead7] hover:bg-white/5"
            }
          `}
        >
          <ListItemIcon
            ...
            `}
            sx={isLogout ? { color: '#ff6b6b !important' } : {}}
          >
    ...
    <div className="h-full w-[300px] bg-[#0f1111] text-[#9ee9dd]">
      ...
      <div className="flex flex-col justify-between h-full py-6 px-4">
        ...
        <div className="space-y-4">
          {menu.map((item, index) => renderMenuItem(item, index, false))}
        </div>

        <Divider className="border-[#2a2a2a] my-4" sx={{ borderColor: '#2a2a2a' }} />

        {/* bottom menu */}
        <div className="space-y-4">
          {menu2.map((item, index) => renderMenuItem(item, index, item.name === "Logout"))}
        </div>
