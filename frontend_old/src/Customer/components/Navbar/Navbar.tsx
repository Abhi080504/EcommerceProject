...
const Navbar = () => {
    ...
    // Fetch cart on mount or when user logs in
    useEffect(() => {
        if (user) {
    ...
    return (
        <AppBar
            ...
            sx={{
                ...
                boxShadow: isGroceryPage ? '0 4px 20px rgba(19, 105, 52, 0.2)' : '0 4px 20px rgba(62, 44, 30, 0.08)',
        ...
        >
            <Toolbar className="max-w-[1600px] mx-auto w-full flex items-center gap-4 lg:gap-8 py-3 px-4 lg:px-10 min-h-[80px]">
                ...
                {/* 2. SEARCH BAR */}
                <div className={`
            flex-1 max-w-[600px] h-[50px] rounded-full flex items-center px-4 gap-3 transition-all duration-300 ml-4 lg:ml-8
        `}
                    style={{
                        backgroundColor: isSearchFocused ? (isGroceryPage ? '#fff' : '#fff') : theme.searchBg,
                        boxShadow: isSearchFocused ? `0 0 0 2px ${theme.accent}` : 'none'
                    }}
                >
                    ...
                    {user?.role === "ROLE_SELLER" ? (
                        <Button
                            ...
                            sx={{
                                ...
                                border: isGroceryPage ? '1px solid rgba(255,255,255,0.2)' : '1px solid rgba(62, 44, 30, 0.1)',
                                '&:hover': { background: 'rgba(0,0,0,0.05)', color: theme.accent, borderColor: theme.accent }
                            }}
