...
const GroceryPage = () => {
    ...
    return (
        ...
        <div className='relative min-h-screen bg-[#F1F8E9] overflow-hidden selection:bg-[#1B5E20] selection:text-white flex flex-col'>
            ...
            {/* CONTENT CONTAINER */}
            <div className={`relative max-w-[1600px] mx-auto px-4 lg:px-10 py-6 space-y-10 transition-all duration-1000 flex-grow ${isLoading ? 'translate-y-10 opacity-0' : 'translate-y-0 opacity-100'}`}>
                    ...
                    <>
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center gap-3">
                                <span className="w-1.5 h-6 md:h-8 bg-[#136934] rounded-full"></span>
                                <Typography variant="h5" sx={{ fontWeight: 900, color: '#1B5E20', fontSize: { xs: '1.2rem', md: '1.5rem' } }}>
                                    Shop by Category
                                </Typography>
                            </div>

                            <div className="flex gap-4 md:gap-5 overflow-x-auto no-scrollbar pb-4 pt-2 px-1">
                                {groceryCategories.map((cat, index) => (
                                    <div
                                        key={index}
                                        onClick={() => scrollToSection(cat.targetId)}
                                        className="relative min-w-[110px] md:min-w-[130px] h-[140px] md:h-[170px] rounded-[24px] cursor-pointer group transition-all duration-500 ease-out hover:-translate-y-2 hover:shadow-[0_15px_30px_-10px_rgba(19,105,52,0.2)]"
                                    >
                                        <div className="absolute inset-0 bg-white border border-[#E8F5E9] rounded-[24px] overflow-hidden z-10 transition-all duration-300 group-hover:border-[#136934]">
                                            <div className="absolute top-3 left-0 right-0 text-center z-20">
                                                <span className="inline-block px-2 md:px-3 py-1 bg-[#F1F8E9] rounded-full text-[#1B5E20] font-bold text-[10px] md:text-xs group-hover:bg-[#136934] group-hover:text-white transition-colors duration-300">
                                                    {cat.label}
                                                </span>
                                            </div>
                                            <img src={cat.image} alt={cat.label} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 ease-in-out group-hover:scale-110 mt-2" />
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                            {banners.map((imgUrl, index) => (
                                <div key={index} className="relative h-[160px] md:h-[200px] rounded-[20px] md:rounded-[28px] overflow-hidden cursor-pointer group shadow-sm hover:shadow-lg transition-all duration-500 hover:-translate-y-1">
                                    <img src={imgUrl} alt="Offer Banner" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                                    <div className="absolute inset-0 bg-gradient-to-tr from-white/30 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
                                </div>
                            ))}
                        </div>
...
const GlassSection = ({ id, title, products }: { id: string, title: string, products: any[] }) => {
    ...
    return (
        <div id={id} className="scroll-mt-32 relative">
            ...
            <div className="bg-white/60 backdrop-blur-xl border border-[#C8E6C9] rounded-[24px] md:rounded-[32px] p-4 md:p-6 shadow-sm hover:shadow-[0_8px_25px_rgba(27,94,32,0.1)] transition-shadow">
